...
Starry.prototype = {
  construct() {
    ...
    this.dots.forEach((dot, m) => {
      _this.dots.forEach((_dot, n) => {
        if (n < m) {
          let line = {
            from: dot,
            to: _dot
  ...
  render() {
    let _this = this
    this.ctx.clearRect(0, 0, this.cx.width, this.cx.height)
    ...
    this.lines.forEach(line => {
      let l = Math.sqrt(Math.pow((line.from.x - line.to.x), 2) + Math.pow((line.from.y - line.to.y), 2))
      let maxl = _this.cx.width / 8
      if (l > maxl)
        return
